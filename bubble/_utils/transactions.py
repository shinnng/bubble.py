...
from bubble._utils.utility_methods import (
    all_in_dict,
    any_in_dict,
)
from bubble.constants import (
...
TRANSACTION_DEFAULTS = {
    "value": 0,
    "data": b"",
    "gas": lambda w3, tx: w3.bub.estimate_gas(tx),
    "gasPrice": lambda w3, tx: w3.bub.generate_gas_price(tx),
    "maxFeePerGas": (
        lambda w3, tx: w3.bub.max_priority_fee
        + (2 * w3.bub.get_block("latest")["baseFeePerGas"])
    ),
    "maxPriorityFeePerGas": lambda w3, tx: w3.bub.max_priority_fee,
    "chainId": lambda w3, tx: w3.bub.chain_id,
}
...
def fill_transaction_defaults(w3: "Web3", transaction: TxParams) -> TxParams:
    ...
    """
    strategy_based_gas_price = w3.bub.generate_gas_price(transaction)
...
def get_block_gas_limit(
    ...
    if block_identifier is None:
        block_identifier = w3.bub.block_number
    block = w3.bub.get_block(block_identifier)
...
def get_buffered_gas_estimate(
    ...
    gas_estimate = w3.bub.estimate_gas(gas_estimate_transaction)
...
def get_required_transaction(w3: "Web3", transaction_hash: _Hash32) -> TxData:
    current_transaction = w3.bub.get_transaction(transaction_hash)
...
def extract_valid_transaction_params(transaction_params: TxData) -> TxParams:
    ...
    # There is always a gasPrice now on bub_getTransaction call for
    ...
    # the bub_sendTransaction call and cause a conflict.
...
def prepare_replacement_transaction(
    ...
    if any_in_dict(DYNAMIC_FEE_TXN_PARAMS, replacement_transaction):
        ...
        generated_gas_price = w3.bub.generate_gas_price(replacement_transaction)
...
def replace_transaction(
    ...
    return w3.bub.send_transaction(new_transaction)
