...
def test_bub_account_create_variation(acct):
    account1 = acct.create()
    account2 = acct.create()
    assert account1 != account2


def test_bub_account_from_key_reproducible(acct, PRIVATE_BYTES):
...
def test_bub_account_from_key_diverge(acct, PRIVATE_BYTES, PRIVATE_BYTES_ALT):
    account1 = acct.from_key(PRIVATE_BYTES)
...
def test_bub_account_from_key_seed_restrictions(acct):
...
def test_bub_account_from_key_properties(acct, PRIVATE_BYTES):
...
def test_bub_account_create_properties(acct):
    account = acct.create()
    assert callable(account.signHash)
    assert callable(account.sign_transaction)
    assert is_checksum_address(account.address)
...
def test_bub_account_recover_transaction_example(acct):
...
def test_bub_account_recover_transaction_with_literal(acct):
...
def test_bub_account_recover_message(acct):
    v, r, s = (
        28,
...
def test_bub_account_recover_signature_bytes(acct, signature_bytes):
    msg = encode_defunct(
...
def test_bub_account_recover_vrs(acct):
    v, r, s = (
        27,
...
def test_bub_account_recover_vrs_standard_v(acct):
    v, r, s = (
        0,
        5634810156301565519126305729385531885322755941350706789683031279718535704513,
        15655399131600894366408541311673616702363115109327707006109616887384920764603,
...
def test_bub_account_sign_transaction(
...
def test_bub_account_sign_transaction_from_eth_test(acct, transaction_info):
...
def test_bub_account_recover_transaction_from_eth_test(acct, transaction):
...
def test_bub_account_encrypt(acct, web3js_key, web3js_password):
...
def test_bub_account_prepared_encrypt(acct, web3js_key, web3js_password):
...
def test_bub_account_sign_and_send_EIP155_transaction_to_eth_tester(
    w3, expected_txn, raw_tx, expected_tx_hash, r, s, v
):
    actual_tx_hash = w3.bub.send_raw_transaction(raw_tx)
    assert actual_tx_hash == expected_tx_hash
    actual_txn = w3.bub.get_transaction(actual_tx_hash)
    for key in (
        "to",
        "nonce",
        "gas",
...
@pytest.fixture()
def async_w3():
    return Web3(AsyncBubereumTesterProvider(), modules={"bub": [AsyncBub]})


@patch("bubble.bub.BaseEth.account", "wired via BaseEth")
def test_account_is_wired_via_base_eth_for_sync_and_async(w3, async_w3):
    ...
    assert w3.bub.account == "wired via BaseEth"
    assert async_w3.bub.account == "wired via BaseEth"


def test_async_eth_account_creates_account(async_w3):
    account = async_w3.bub.account.create()
