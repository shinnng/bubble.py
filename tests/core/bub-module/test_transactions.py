...
from bubble.middleware.simulate_unmined_transaction import (
...
def test_bub_tester_send_transaction_validation(w3, transaction):
    ...
    txn_hash = w3.bub.send_transaction(transaction)
    receipt = w3.bub.wait_for_transaction_receipt(txn_hash, timeout=RECEIPT_TIMEOUT)
...
def test_send_transaction_with_valid_chain_id(w3, make_chain_id, expect_success):
    ...
    if expect_success:
        ...
        with pytest.raises(Web3ValidationError) as exc_info:
            w3.bub.send_transaction(transaction)
...
@pytest.mark.parametrize(
    ...
    (
        (
            "registered-name-1.bub",
            "not-a-registered-name.bub",
        ),
        (
            "not-a-registered-name.bub",
            "registered-name-1.bub",
...
def test_send_transaction_with_invalid_ens_names(w3, to, _from):
    with ens_addresses(
        w3,
        [
            ("registered-name-1.bub", w3.bub.accounts[1]),
        ],
    ):
        transaction = {
            "to": to,
...
def test_send_transaction_with_ens_names(w3):
    with ens_addresses(
        ...
        [
            ("registered-name-1.bub", w3.bub.accounts[1]),
            ("registered-name-2.bub", w3.bub.accounts[0]),
        ],
    ):
        transaction = {
            "to": "registered-name-1.bub",
            "chainId": w3.bub.chain_id,
            "from": "registered-name-2.bub",
        }
...
def test_wait_for_missing_receipt(w3):
    with pytest.raises(TimeExhausted):
        w3.bub.wait_for_transaction_receipt(b"\0" * 32, timeout=RECEIPT_TIMEOUT)
...
def test_passing_string_to_to_hex(w3):
    with pytest.raises(TimeExhausted):
        ...
        w3.bub.wait_for_transaction_receipt(transaction_hash, timeout=RECEIPT_TIMEOUT)
...
def test_unmined_transaction_wait_for_receipt(w3):
    ...
    with pytest.raises(TransactionNotFound):
        w3.bub.get_transaction_receipt(txn_hash)
