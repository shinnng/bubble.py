...
from bubble._utils.contract_sources.contract_data.arrays_contract import (
...
from bubble._utils.contract_sources.contract_data.contract_caller_tester import (
    CONTRACT_CALLER_TESTER_DATA,
)
from bubble._utils.contract_sources.contract_data.event_contracts import (
    EVENT_CONTRACT_DATA,
    INDEXED_EVENT_CONTRACT_DATA,
)
from bubble._utils.contract_sources.contract_data.fallback_function_contract import (
...
from bubble._utils.contract_sources.contract_data.payable_tester import (
    PAYABLE_TESTER_CONTRACT_DATA,
)
from bubble._utils.contract_sources.contract_data.receive_function_contracts import (
    NO_RECEIVE_FUNCTION_CONTRACT_DATA,
    RECEIVE_FUNCTION_CONTRACT_DATA,
)
from bubble._utils.contract_sources.contract_data.reflector_contracts import (
...
from bubble._utils.contract_sources.contract_data.string_contract import (
    STRING_CONTRACT_DATA,
)
from bubble._utils.contract_sources.contract_data.tuple_contracts import (
...
def math_contract_factory(w3):
    return w3.bub.contract(**MATH_CONTRACT_DATA)
...
def simple_constructor_contract_factory(w3):
    return w3.bub.contract(**SIMPLE_CONSTRUCTOR_CONTRACT_DATA)


@pytest.fixture
def contract_with_constructor_args_factory(w3):
    return w3.bub.contract(**CONSTRUCTOR_WITH_ARGUMENTS_CONTRACT_DATA)


@pytest.fixture
def non_strict_contract_with_constructor_args_factory(w3_non_strict_abi):
    return w3_non_strict_abi.bub.contract(**CONSTRUCTOR_WITH_ARGUMENTS_CONTRACT_DATA)


@pytest.fixture
def contract_with_constructor_address_factory(w3):
    return w3.bub.contract(**CONSTRUCTOR_WITH_ADDRESS_ARGUMENT_CONTRACT_DATA)
...
def address_reflector_contract(w3, address_conversion_func):
    address_reflector_contract_factory = w3.bub.contract(
        **ADDRESS_REFLECTOR_CONTRACT_DATA
    )
...
def string_contract_factory(w3, string_contract_data):
    return w3.bub.contract(**STRING_CONTRACT_DATA)
...
def non_strict_string_contract(
    ...
    _non_strict_string_contract_factory = w3_non_strict_abi.bub.contract(
        **string_contract_data
    )
    return deploy(
        w3_non_strict_abi,
...
def non_strict_emitter(
    ...
    non_strict_emitter_contract_factory = w3_non_strict_abi.bub.contract(
        **emitter_contract_data
    )
...
def event_contract(
    ...
    event_contract_factory = w3.bub.contract(**EVENT_CONTRACT_DATA)
    deploy_txn_hash = event_contract_factory.constructor().transact(
        {"from": w3.bub.coinbase, "gas": 1000000}
    )
    deploy_receipt = wait_for_transaction(w3, deploy_txn_hash)
    contract_address = address_conversion_func(deploy_receipt["contractAddress"])
...
def indexed_event_contract(
    ...
    indexed_event_contract_factory = w3.bub.contract(**INDEXED_EVENT_CONTRACT_DATA)
...
def arrays_contract(w3, address_conversion_func):
    arrays_contract_factory = w3.bub.contract(**ARRAYS_CONTRACT_DATA)
    return deploy(
        w3,
...
def non_strict_arrays_contract(w3_non_strict_abi, address_conversion_func):
    non_strict_arrays_contract_factory = w3_non_strict_abi.bub.contract(
        **ARRAYS_CONTRACT_DATA
    )
    return deploy(
        w3_non_strict_abi,
...
def payable_tester_contract(w3, address_conversion_func):
    payable_tester_contract_factory = w3.bub.contract(**PAYABLE_TESTER_CONTRACT_DATA)
...
def fixed_reflector_contract(w3, address_conversion_func):
    fixed_reflector_contract_factory = w3.bub.contract(
...
def fallback_function_contract(w3, address_conversion_func):
    fallback_function_contract_factory = w3.bub.contract(
        **FALLBACK_FUNCTION_CONTRACT_DATA
    )
...
def receive_function_contract(w3, address_conversion_func):
    receive_function_contract_factory = w3.bub.contract(
        **RECEIVE_FUNCTION_CONTRACT_DATA
    )
...
def no_receive_function_contract(w3, address_conversion_func):
    no_receive_function_contract_factory = w3.bub.contract(
        **NO_RECEIVE_FUNCTION_CONTRACT_DATA
    )
...
def contract_caller_tester_contract(w3, address_conversion_func):
    contract_caller_tester_contract_factory = w3.bub.contract(
        **CONTRACT_CALLER_TESTER_DATA
    )
...
def tuple_contract_with_decode_tuples(w3, address_conversion_func):
    tuple_contract_factory = w3.bub.contract(**TUPLE_CONTRACT_DATA_DECODE_TUPLES)
    return deploy(w3, tuple_contract_factory, address_conversion_func)


@pytest.fixture
def nested_tuple_contract_with_decode_tuples(w3, address_conversion_func):
    nested_tuple_contract_factory = w3.bub.contract(
        **NESTED_TUPLE_CONTRACT_DATA_DECODE_TUPLES
    )
    return deploy(w3, nested_tuple_contract_factory, address_conversion_func)
...
def async_math_contract_factory(async_w3):
    return async_w3.bub.contract(**MATH_CONTRACT_DATA)
...
def async_simple_constructor_contract_factory(async_w3):
    return async_w3.bub.contract(**SIMPLE_CONSTRUCTOR_CONTRACT_DATA)


@pytest.fixture
def async_constructor_with_args_contract_factory(async_w3):
    return async_w3.bub.contract(**CONSTRUCTOR_WITH_ARGUMENTS_CONTRACT_DATA)


@pytest.fixture
def async_non_strict_constructor_with_args_contract_factory(async_w3_non_strict_abi):
    return async_w3_non_strict_abi.bub.contract(
        **CONSTRUCTOR_WITH_ARGUMENTS_CONTRACT_DATA
    )
...
def async_constructor_with_address_arg_contract_factory(async_w3):
    return async_w3.bub.contract(**CONSTRUCTOR_WITH_ADDRESS_ARGUMENT_CONTRACT_DATA)
...
async def async_constructor_with_address_argument_contract(
    ...
    async_constructor_with_address_arg_factory = async_w3.bub.contract(
        **CONSTRUCTOR_WITH_ADDRESS_ARGUMENT_CONTRACT_DATA
    )
    return await async_deploy(
        async_w3,
...
async def async_address_reflector_contract(async_w3, address_conversion_func):
    async_address_reflector_contract_factory = async_w3.bub.contract(
        **ADDRESS_REFLECTOR_CONTRACT_DATA,
    )
    return await async_deploy(
...
def async_string_contract_factory(async_w3):
    return async_w3.bub.contract(**STRING_CONTRACT_DATA)
...
async def async_arrays_contract(async_w3, address_conversion_func):
    async_arrays_contract_factory = async_w3.bub.contract(**ARRAYS_CONTRACT_DATA)
    return await async_deploy(
        async_w3,
...
async def async_non_strict_arrays_contract(
    ...
    async_non_strict_arrays_contract_factory = async_w3_non_strict_abi.bub.contract(
        **ARRAYS_CONTRACT_DATA,
    )
    return await async_deploy(
...
async def async_payable_tester_contract(async_w3, address_conversion_func):
    async_payable_tester_contract_factory = async_w3.bub.contract(
        **PAYABLE_TESTER_CONTRACT_DATA
    )
    return await async_deploy(
...
async def async_fixed_reflector_contract(async_w3, address_conversion_func):
    async_fixed_reflector_contract_factory = async_w3.bub.contract(
        abi=FIXED_REFLECTOR_CONTRACT_ABI, bytecode=FIXED_REFLECTOR_CONTRACT_BYTECODE
    )
    return await async_deploy(
...
async def async_fallback_function_contract(async_w3, address_conversion_func):
    async_fallback_function_contract_factory = async_w3.bub.contract(
        **FALLBACK_FUNCTION_CONTRACT_DATA
    )
    return await async_deploy(
...
async def async_no_receive_function_contract(async_w3, address_conversion_func):
    async_no_receive_function_contract_factory = async_w3.bub.contract(
        **NO_RECEIVE_FUNCTION_CONTRACT_DATA
    )
    return await async_deploy(
...
async def async_receive_function_contract(async_w3, address_conversion_func):
    async_receive_function_contract_factory = async_w3.bub.contract(
        **RECEIVE_FUNCTION_CONTRACT_DATA
    )
    return await async_deploy(
...
async def async_contract_caller_tester_contract(async_w3, address_conversion_func):
    async_contract_caller_tester_contract_factory = async_w3.bub.contract(
        **CONTRACT_CALLER_TESTER_DATA
    )
    return await async_deploy(
        async_w3,
...
async def async_revert_contract(async_w3, address_conversion_func):
    async_revert_contract_factory = async_w3.bub.contract(**REVERT_CONTRACT_DATA)
    return await async_deploy(
...
async def async_tuple_contract(async_w3, address_conversion_func):
    async_tuple_contract_factory = async_w3.bub.contract(**TUPLE_CONTRACT_DATA)
    return await async_deploy(
...
async def async_nested_tuple_contract(async_w3, address_conversion_func):
    async_nested_tuple_contract_factory = async_w3.bub.contract(
        **NESTED_TUPLE_CONTRACT_DATA
    )
    return await async_deploy(
...
async def async_tuple_contract_with_decode_tuples(async_w3, address_conversion_func):
    async_tuple_contract_factory = async_w3.bub.contract(
        **TUPLE_CONTRACT_DATA_DECODE_TUPLES
    )
    return await async_deploy(
        async_w3, async_tuple_contract_factory, address_conversion_func
