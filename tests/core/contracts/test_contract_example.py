...
# of how to write unit tests with bubble.py
import pytest
...
@pytest.fixture
def tester_provider():
    return BubbleTesterProvider()


@pytest.fixture
def eth_tester(tester_provider):
    return tester_provider.bubble_tester
...
@pytest.fixture
def foo_contract(bub_tester, w3):
    ...
    deploy_address = bub_tester.get_accounts()[0]
    ...
    FooContract = w3.bub.contract(abi=abi, bytecode=bytecode)
    ...
    tx_receipt = w3.bub.wait_for_transaction_receipt(tx_hash, 180)
...
def test_can_update_greeting(w3, foo_contract):
    ...
    tx_hash = foo_contract.functions.setBar("testing contracts is easy").transact(
        {
            "from": w3.bub.accounts[1],
        }
    )
    w3.bub.wait_for_transaction_receipt(tx_hash, 180)
...
def test_updating_greeting_emits_event(w3, foo_contract):
    ...
    receipt = w3.bub.wait_for_transaction_receipt(tx_hash, 180)
...
def async_eth_tester():
    return AsyncBubereumTesterProvider().ethereum_tester
...
async def async_foo_contract(async_w3):
    ...
    async_bub_tester_accounts = await async_w3.bub.accounts
    deploy_address = async_bub_tester_accounts[0]
    ...
    # Create our contract class.
    FooContract = async_w3.bub.contract(abi=abi, bytecode=bytecode)
    # issue a transaction to deploy the contract.
    ...
    # wait for the transaction to be mined
    tx_receipt = await async_w3.bub.wait_for_transaction_receipt(tx_hash, 180)
    # instantiate and return an instance of our contract.
    return FooContract(tx_receipt.contractAddress)
...
async def test_async_can_update_greeting(async_w3, async_foo_contract):
    ...
    tx_hash = await async_foo_contract.functions.setBar(
        ...
        {
            "from": async_bub_tester_accounts[1],
        }
    )
    await async_w3.bub.wait_for_transaction_receipt(tx_hash, 180)
...
async def test_async_updating_greeting_emits_event(async_w3, async_foo_contract):
    ...
    receipt = await async_w3.bub.wait_for_transaction_receipt(tx_hash, 180)
