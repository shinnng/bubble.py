...
def test_instantiate_existing_filter(w3, sleep_interval, wait_for_block, filter_id):
    with pytest.raises(TypeError):
        w3.bub.filter("latest", filter_id)
    with pytest.raises(TypeError):
        w3.bub.filter("latest", filter_id=filter_id)
    with pytest.raises(TypeError):
        w3.bub.filter(filter_params="latest", filter_id=filter_id)

    block_filter = w3.bub.filter(filter_id=filter_id)
...
async def test_async_instantiate_existing_filter(
    ...
    with pytest.raises(TypeError):
        await async_w3.bub.filter("latest", async_filter_id)
    with pytest.raises(TypeError):
        await async_w3.bub.filter("latest", filter_id=async_filter_id)
    with pytest.raises(TypeError):
        await async_w3.bub.filter(filter_params="latest", filter_id=async_filter_id)

    block_filter = await async_w3.bub.filter(filter_id=async_filter_id)
    ...
    for n in range(current_block, current_block + 3):
        next_block = await async_w3.bub.get_block(n + 1)
