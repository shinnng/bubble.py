...
from bubble.middleware.stalecheck import (
    StaleBlockchain,
    _is_fresh,
...
def test_stalecheck_pass(request_middleware):
    with patch("bubble.middleware.stalecheck._is_fresh", return_value=True):
...
def test_stalecheck_fail(request_middleware, now):
    with patch("bubble.middleware.stalecheck._is_fresh", return_value=False):
        request_middleware.web3.bub.get_block.return_value = stub_block(now)
...
def test_stalecheck_ignores_get_by_block_methods(request_middleware, rpc_method):
    ...
    with patch("bubble.middleware.stalecheck._is_fresh", side_effect=[False, True]):
        request_middleware(rpc_method, [])
        assert not request_middleware.web3.bub.get_block.called
...
def test_stalecheck_calls_is_fresh_with_empty_cache(
    ...
    with patch(
        "bubble.middleware.stalecheck._is_fresh", side_effect=[False, True]
    ) as fresh_spy:
        block = object()
        request_middleware.web3.bub.get_block.return_value = block
        request_middleware("", [])
...
def test_stalecheck_adds_block_to_cache(request_middleware, allowable_delay):
    with patch(
        "bubble.middleware.stalecheck._is_fresh", side_effect=[False, True, True]
    ) as fresh_spy:
        block = object()
        request_middleware.web3.bub.get_block.return_value = block
...
async def test_async_stalecheck_fail(request_async_middleware, now):
    with patch("bubble.middleware.stalecheck._is_fresh", return_value=False):
        request_async_middleware.web3.bub.get_block.return_value = stub_block(now)
        with pytest.raises(StaleBlockchain):
            await request_async_middleware("", [])


@pytest.mark.asyncio
@pytest.mark.parametrize("rpc_method", ["bub_getBlockByNumber"])
@min_version
async def test_async_stalecheck_ignores_get_by_block_methods(
    ...
    with patch("bubble.middleware.stalecheck._is_fresh", side_effect=[False, True]):
        ...
        assert not request_async_middleware.web3.bub.get_block.called
...
async def test_async_stalecheck_calls_is_fresh_with_empty_cache(
    ...
    with patch(
        ...
        block = object()
        request_async_middleware.web3.bub.get_block.return_value = block
        await request_async_middleware("", [])
        cache_call, live_call = fresh_spy.call_args_list
        assert cache_call[0] == (None, allowable_delay)
        assert live_call[0] == (block, allowable_delay)
