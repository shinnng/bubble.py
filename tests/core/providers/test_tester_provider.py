...
def test_tester_provider_creates_a_block() -> None:
    provider = BubbleTesterProvider()
    accounts = provider.make_request("bub_accounts", [])
    a, b = accounts["result"][:2]
    current_block = provider.make_request("bub_blockNumber", [])
...
def test_bub_tester_provider_properly_handles_eth_tester_error_messages(
    mocker,
    exception_case,
):
    mocker.patch(
    ...
    with pytest.raises(
        ...
        provider.make_request(RPCEndpoint("bub_blockNumber"), [])
