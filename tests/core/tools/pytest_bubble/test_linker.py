...
from bubble.tools.pytest_bubble.exceptions import (
    DeployerError,
)
...
def test_linker_with_from(escrow_deployer, w3):
    escrow_strategy = linker(
        deploy("SafeSendLib"),
        link("Escrow", "SafeSendLib"),
        deploy("Escrow", w3.bub.accounts[0], transaction={"from": w3.bub.accounts[5]}),
    )
    escrow_deployer.register_strategy("Escrow", escrow_strategy)
    linked_escrow_package = escrow_deployer.deploy("Escrow")
    escrow_instance = linked_escrow_package.deployments.get_instance("Escrow")
    assert escrow_instance.functions.sender().call() == w3.bub.accounts[5]


def test_linker_with_callback(escrow_deployer, w3):
    sender = w3.bub.accounts[0]
    recipient = w3.bub.accounts[5]
    ...
    escrow_deployer.register_strategy("Escrow", escrow_strategy)
    assert w3.bub.get_balance(recipient) == w3.to_wei("1000000", "ether")
    linked_escrow_package = escrow_deployer.deploy("Escrow")
    escrow_instance = linked_escrow_package.deployments.get_instance("Escrow")
    assert escrow_instance.functions.sender().call() == sender
    assert w3.bub.get_balance(recipient) == w3.to_wei("1000001", "ether")
