...
def test_set_text_fails_with_bad_address(ens):
    address = ens.w3.bub.accounts[2]
    ens.setup_address("tester.bub", address)
    zero_address = "0x" + "00" * 20
    with pytest.raises(EthUtilsValidationError):
        ens.set_text(
            "tester.bub", "url", "http://example.com", transact={"from": zero_address}
        )

    # teardown
    ens.setup_address("tester.bub", None)


def test_set_text_pass_in_transaction_dict(ens):
    address = ens.w3.bub.accounts[2]
    ens.setup_address("tester.bub", address)
    ens.set_text("tester.bub", "url", "http://example.com", transact={"from": address})
    ens.set_text(
        "tester.bub",
        "avatar",
        "example.jpeg",
    ...
    assert ens.get_text("tester.bub", "url") == "http://example.com"
    assert ens.get_text("tester.bub", "avatar") == "example.jpeg"
...
def test_get_text(ens, key, expected):
    ...
    ens.set_text("tester.bub", key, expected)
    assert ens.get_text("tester.bub", key) == expected
...
def test_get_text_for_resolver_with_unsupported_function(ens):
    with pytest.raises(UnsupportedFunction, match="does not support `text` function"):
        ens.get_text("simple-resolver.bub", "any_key")
...
async def test_async_set_text_fails_with_bad_address(async_ens):
    ...
    address = accounts[2]
    await async_ens.setup_address("tester.bub", address)
    zero_address = "0x" + "00" * 20
    with pytest.raises(EthUtilsValidationError):
    ...
    # teardown
    await async_ens.setup_address("tester.bub", None)
...
async def async_test_set_text_pass_in_transaction_dict(async_ens):
    ...
    await async_ens.setup_address("tester.bub", address)
    await async_ens.set_text(
        "tester.bub", "url", "http://example.com", transact={"from": address}
    )
    await async_ens.set_text(
        "tester.bub",
        "avatar",
    ...
    assert await async_ens.get_text("tester.bub", "url") == "http://example.com"
    assert await async_ens.get_text("tester.bub", "avatar") == "example.jpeg"
...
async def test_async_get_text(async_ens, key, expected):
    ...
    assert address == owner
    await async_ens.set_text("tester.bub", key, expected)
    assert await async_ens.get_text("tester.bub", key) == expected
...
async def test_async_get_text_for_resolver_with_unsupported_function(async_ens):
    with pytest.raises(UnsupportedFunction, match="does not support `text` function"):
        await async_ens.get_text("simple-resolver.bub", "any_key")
